...
**M0.1 FROZEN `PASS` · M1.0 independent re-judge `PASS WITH NOTES` (real-input smoke begun, incomplete; not frozen) · M1.1 spec FROZEN, CP1 CLOSED (judge `PASS WITH NOTES`, no blockers) · CP2 (deterministic text normalization) merged (PR #4) + hosted CI `PASS` — CP2 formally CLOSED · CP3 is the next authorized checkpoint · CP4–CP6 not implemented.**
...
- Branch: `main` @ `48a704874ea5b2e68ad8680a8a70cb3a52b733f8` (CP2 merge, PR #4) · verify HEAD with `git rev-parse HEAD`
...
- **M1.1 CP3 (Routine) — next authorized checkpoint:** exact ordered matching — match a passage's normalized token sequence to a contiguous run of transcript entries, yielding one `SourceRange` via `createSourceRange` (`start = entry[i].startUs`, `end = entry[j].endUs`), building on the CP2 `normalizeSelectionText` foundation. Verified by CI + boundary guard + property tests + self-review; no judge. CP2 required no architecture-guard change, so CP1 judge note 1 (a `selection`-specific negative-edge test) remains a future coverage opportunity; the branded-`Confidence` note stays deferred to the resolver-result contract (CP6).
- **Then CP4–CP6 (Routine, not implemented):** fuzzy match → ambiguity/alternatives → result contract. Verified by CI + boundary guard + property tests + self-review; no judge.
